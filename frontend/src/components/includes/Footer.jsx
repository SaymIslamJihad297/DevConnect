...
const Footer = () => {
    const { isLoggedIn, LogOutApi, setIsLoggedIn } = useContext(apiContext);
    const LogOutFunction = async () => {
        const response = await api.get(LogOutApi);
    ...
    return (
        <div className='flex justify-center'>
            <div
                className="flex fixed bottom-5 justify-around max-w-96 gap-4 items-center px-4 py-1 bg-black rounded-[15px] ring-1 ring-white"
            >
                <div
                ...
                {!isLoggedIn && <NavLink to="/login">
                    <div
                        className="relative group hover:cursor-pointer hover:bg-slate-800 p-2 rounded-full transition-all duration-500"
                    >
                        <i className="fa-solid fa-user-plus text-white"></i> <div
                            className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-max px-2 py-1 text-white bg-black rounded-md opacity-0 transform scale-50 transition-all duration-500 group-hover:opacity-100 group-hover:scale-100"
                        >
                            Login
                        </div>
                ...
                {isLoggedIn &&
                    ...
                    >
                        <button onClick={LogOutFunction}>
                            <i className="fa-solid fa-right-from-bracket text-white"></i></button> <div
                                className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-max px-2 py-1 text-white bg-black rounded-md opacity-0 transform scale-50 transition-all duration-500 group-hover:opacity-100 group-hover:scale-100"
                            >
                            Logout
                        </div>
